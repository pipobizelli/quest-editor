...
- **Play-mode hooks**: In play mode, clicking a monster (or discovered trap) **selects** it; pressing **Delete/Backspace** then fires `monster:killed` / `trap:disarmed` (carries the element) — an **intercept** hook: the editor does NOT remove it. The host reacts (e.g. opens a "who killed it?" modal) and then removes via `handle.removeElement(id)`. Adding a new play hook = new `EditorEvent` variant + a trigger in `QuestEditor`/store. See the `apps/playground` App for a reference host implementation.
- **Play-mode discovery & search**: A discovery layer (`discoveredElements: Set<id>` in the store, reset on mode change) hides **room traps** and **secret doors** even after their room is revealed — they're found only by searching. Corridor traps are unchanged (revealed with the corridor tile). The host calls `handle.searchRoom(groupId, 'treasure'|'traps'|'secret')`: `traps`/`secret` reveal matching hidden elements and emit `search:traps`/`search:secret` with the `found` array; `treasure` is abstract (emits `search:treasure`, no board element). Selecting a discovered trap and pressing Delete fires the `trap:disarmed` intercept hook (host attributes + removes).
- **Hero placement (play mode)**: `handle.placeHeroes(heroes)` auto-places the party on free tiles around the stairway (the stairway marker itself doesn't block), emitting `heroes:placed`. With no stairway it enters click-to-place: emits `heroes:need-placement`, sets `placingHeroes`, and each board click drops the next hero (`placeNextHeroAt`) until done. Heroes (type `hero`) are always visible in play mode — never fog-hidden.
